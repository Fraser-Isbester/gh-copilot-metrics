...
def test_upload_to_bigquery_missing_env_vars():
    ...
    with (
        ...
        pytest.raises(
            ValueError,
            match="GCP_PROJECT_ID and BQ_DATASET environment variables must be set",
        ),
    ):
        upload_to_bigquery([], [])
...
def test_upload_to_bigquery_success():
    ...
    with (
        ...
        patch(
            "pilot_metrics.bigquery_uploader.bigquery.Client", return_value=mock_client
        ),
        patch(
            "pilot_metrics.bigquery_uploader.bigquery.Dataset",
            return_value=mock_dataset,
        ),
        patch(
            "pilot_metrics.bigquery_uploader.bigquery.Table", return_value=mock_table
        ),
    ):
        ...
        # Verify client was created (constructor doesn't take project directly)
